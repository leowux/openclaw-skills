...
```
截图后将 `/tmp/xhs_qrcode.png` 发送给用户，提示：**"请用小红书 App 扫描二维码完成登录"**。

用户扫码完成后，等待5秒，再次检查登录状态确认成功。
...
screencapture -x /tmp/xhs_qrcode.png
```
将截图发送给用户，提示扫码。用户扫码后等待5秒，再用 `status` 命令确认登录成功。
